...
2.  **Data Collection:**
    ...
    a.  Traffic volume counts during should be done in peak traffic
        periods.
        -   *Group BLUE* : Collect data for Morning Peak (7.30-9.30AM)

        -   *Group RED* : Collect data for Evening peak (4.30-6.30AM)
    ...
    c.  Traffic counts should be collected on a "typical" day; thus,
        they should be done on Tuesdays, Wednesdays, and Thursdays.
        Avoid doing traffic counts on Mondays and Fridays. A traffic
        count template is provided in the TMC lab session.

3.  **Capacity analyses**:

    a.  Present level of service (LOS) for existing traffic condition
        and traffic control using HCS 2010 and Synchro. Compare results
        (e.g., delay) from both software.

    b.  Analyze the LOS in Synchro if traffic control at the
        intersection is (i) TWSC and (ii) Roundabout
...
Tech memo is due from each team on **TBA**.
